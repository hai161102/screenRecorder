...
class FloatingView extends FrameLayout implements ViewTreeObserver.OnPreDrawListener {
    ...
    public boolean onPreDraw() {
        ...
        if (mInitY == DEFAULT_Y) {
            mInitY = mMetrics.heightPixels / 2 - getMeasuredHeight();
        }
        ...
        if (mMoveDirection == FloatingViewManager.MOVE_DIRECTION_NONE) {
            ...
            // 初期位置から画面端に移動
            moveToEdge(mInitX, mInitY, false);
        }
        mIsDraggable = true;
    ...
    void setXY(int floatingViewX, int floatingViewY) {
        this.mParams.x = floatingViewX;
        this.mParams.y = floatingViewY;
    }
